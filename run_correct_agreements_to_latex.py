#!/usr/bin/env python3
"""Run enhanced extraction on the correct credit agreements and generate LaTeX output."""
...
def extraction_to_latex(extraction, doc_name):
    ...
    if extraction.get('obligations'):
        latex.append("\\subsection{Parties and Commitments}")
        
        # Parties table
        if extraction['obligations'].get('parties'):
            ...
            for party in extraction['obligations']['parties']:
                latex.append(f"{escape_latex(party['role'])} & {escape_latex(party['name'])} & {escape_latex(party.get('jurisdiction') or 'N/A')} \\\\")
            latex.append("\\hline")
            latex.append("\\end{tabular}")
            latex.append("")
        
        # Commitments
        if extraction['obligations'].get('commitments'):
            ...
            for commit in extraction['obligations']['commitments']:
                ...
                if commit.get('purpose'):
                    latex.append(f"  \\begin{{itemize}}")
                    latex.append(f"  \\item Purpose: {escape_latex(commit['purpose'])}")
    ...
    if extraction.get('pricing'):
        ...
        if extraction['pricing'].get('base_interest_rate'):
            ...
            if rate.get('benchmark_definition'):
        ...
        if extraction['pricing'].get('performance_pricing'):
    ...
    if extraction.get('conditions') and extraction['conditions'].get('financial_covenants'):
    ...
    if extraction.get('repayment') and extraction['repayment'].get('mandatory_prepayments'):
    ...
    if extraction.get('fees_and_costs') and extraction['fees_and_costs'].get('fees'):
    ...
    if extraction.get('administrative') and extraction['administrative'].get('governing_terms'):
        ...
        if terms.get('amendment_threshold'):
            latex.append(f"\\item \\textbf{{Amendment Threshold:}} {terms['amendment_threshold']}")
        
        if terms.get('required_lenders_definition'):
            latex.append(f"  \\begin{{itemize}}")
    ...
    # Summary of Definitions Captured
    latex.append("\\subsection{Summary of Definitions Captured}")
    definition_count = count_definitions_in_extraction(extraction)
    latex.append(f"This extraction captured \\textbf{{{definition_count} definitions}} to explain the economic terms.")
    latex.append("")
    
    return '\n'.join(latex)

def count_definitions_in_extraction(obj, count=0):
    """Recursively count definition fields in the extraction."""
    if isinstance(obj, dict):
        for key, value in obj.items():
            if 'definition' in key.lower() and value:
                count += 1
            count = count_definitions_in_extraction(value, count)
    elif isinstance(obj, list):
        for item in obj:
            count = count_definitions_in_extraction(item, count)
    return count
...
def create_full_latex_document(extractions):
    ...
    latex.append("")
    latex.append("\\title{Credit Agreement Extractions with Definitions\\\\[0.5em]")
    latex.append("{\\large Enhanced Prompt Testing}}")
    ...
    latex.append("This document presents structured extractions from two major credit agreements using an enhanced prompt that captures definitions alongside economic terms.")
    latex.append("")
    latex.append("\\textbf{Key Feature:} Definitions are \\colorbox{yellow}{highlighted in yellow} throughout the document, showing how the enhanced prompt captures the meaning of financial terms directly from the source documents.")
    latex.append("")
    latex.append("\\newpage")
    latex.append("")
...
def main():
    ...
    for doc_name, doc_path in test_docs:
        ...
        try:
            ...
            json_dir = Path(f"correct_agreements_{timestamp}")
            ...
            # Count definitions
            def_count = count_definitions_in_extraction(extraction)
            print(f"Definitions captured: {def_count}")
    ...
    if extractions:
        ...
        # Save LaTeX file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        latex_file = Path(f"credit_agreements_enhanced_{timestamp}.tex")
