...
def _parse_args(argv: List[str] | None = None) -> argparse.Namespace:  # noqa: D401
    ...
    extract_cmd = sub.add_parser(
        "extract",
        help="Run the full pipeline on a file or directory of exhibits (unified path)",
    )
...
def main(argv: List[str] | None = None) -> None:  # noqa: D401
    ...
    if args.command == "run":
        ...
        rows = _run_for_filing(
            ...
            reset_memory=getattr(args, "reset_memory", False),
        )

        for row in rows:
            console.print_json(json.dumps(row.data))
